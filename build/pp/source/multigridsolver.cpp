...
#line 1 "/home/dalab/workspace/fluid/manta/source/multigridsolver.cpp"
/******************************************************************************
 *
...
template<class T>
void MultiGridSolver::GridStorage<T>::free() {
	if (used != 0)
...
template<class T>
T* MultiGridSolver::GridStorage<T>::get(Vec3i size) {
	if ((int)grids.size() <= used) {
		debMsg("MultiGridSolver::GridStorage::get Allocating new "<<size.x<<","<<size.y<<","<<size.z<<" ",3); 
...
template<class T>
void MultiGridSolver::GridStorage<T>::release(T* ptr) {
...
template<> int* MultiGridSolver::getGridPointer<int>() {
	return mGridsInt.get(mGridSize);    
}
template<> Real* MultiGridSolver::getGridPointer<Real>() {
	return mGridsReal.get(mGridSize);    
}
template<> Vec3* MultiGridSolver::getGridPointer<Vec3>() {
	return mGridsVec.get(mGridSize);    
}
template<> Vec4* MultiGridSolver::getGridPointer<Vec4>() {
	return mGridsVec4.get(mGridSize);    
}
template<> void MultiGridSolver::freeGridPointer<int>(int *ptr) {
	mGridsInt.release(ptr);
}
template<> void MultiGridSolver::freeGridPointer<Real>(Real* ptr) {
	mGridsReal.release(ptr);
}
template<> void MultiGridSolver::freeGridPointer<Vec3>(Vec3* ptr) {
	mGridsVec.release(ptr);
}
template<> void MultiGridSolver::freeGridPointer<Vec4>(Vec4* ptr) {
	mGridsVec4.release(ptr);
}

// 4d data (work around for now, convert to 1d length)

template<> int* MultiGridSolver::getGrid4dPointer<int>() {
	return mGrids4dInt.get( Vec3i(mGridSize[0]*mGridSize[1],mGridSize[2],mFourthDim) );    
}
template<> Real* MultiGridSolver::getGrid4dPointer<Real>() {
	return mGrids4dReal.get( Vec3i(mGridSize[0]*mGridSize[1],mGridSize[2],mFourthDim) );    
}
template<> Vec3* MultiGridSolver::getGrid4dPointer<Vec3>() {
	return mGrids4dVec.get( Vec3i(mGridSize[0]*mGridSize[1],mGridSize[2],mFourthDim) );    
}
template<> Vec4* MultiGridSolver::getGrid4dPointer<Vec4>() {
	return mGrids4dVec4.get( Vec3i(mGridSize[0]*mGridSize[1],mGridSize[2],mFourthDim) );    
}
template<> void MultiGridSolver::freeGrid4dPointer<int>(int *ptr) {
	mGrids4dInt.release(ptr);
}
template<> void MultiGridSolver::freeGrid4dPointer<Real>(Real* ptr) {
	mGrids4dReal.release(ptr);
}
template<> void MultiGridSolver::freeGrid4dPointer<Vec3>(Vec3* ptr) {
	mGrids4dVec.release(ptr);
}
template<> void MultiGridSolver::freeGrid4dPointer<Vec4>(Vec4* ptr) {
	mGrids4dVec4.release(ptr);
}

//******************************************************************************
// MultiGridSolver members

MultiGridSolver::MultiGridSolver(Vec3i gridsize, int dim, int fourthDim)
...
PbClass* MultiGridSolver::create(PbType t, PbTypeVec T, const string& name) {        
#	if NOPYTHON!=1
...
void MultiGridSolver::step() {
	...
	updateQtGui(true, mFrame,mTimeTotal, "MultiGridSolver::step");
}
...
void MultiGridSolver::printMemInfo() {
...
void MultiGridSolver::adaptTimestep(Real maxVel)
{
