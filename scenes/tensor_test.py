...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    shape = [BATCH_SIZE, resC*resF, resC*resF, VECTOR_DIM]
    coarse_new_node = tf.placeholder(tf.float32, shape=shape)
    coarse_old_node = tf.placeholder(tf.float32, shape=shape)
    global_node = tf.placeholder(tf.float32, shape=shape)
    truth_node = tf.placeholder(tf.float32, shape=[BATCH_SIZE, NUM_LABELS])

    prediction = global_node + coarse_new_node - coarse_old_node
    prediction = tf.reshape(prediction, [BATCH_SIZE, NUM_LABELS])
    loss = tf.reduce_sum(tf.abs(prediction - truth_node))/(tf.reduce_sum(tf.abs(truth_node))+tf.reduce_sum(tf.abs(prediction)))
    ...
    with tf.Session() as s:
        ...
        for step in xrange(int(num_epochs * train_size / BATCH_SIZE)):
            ...
            feed_dict = {
                ...
                truth_node: train_truth_data[offset:(offset+BATCH_SIZE)]}
            # Run the graph and fetch some of the nodes.
            l = s.run(loss, feed_dict = feed_dict)
            result = result + l
            cnt = cnt + 1
            print 'step %i, current loss: %.4f, avg. loss: %.4f' % (cnt-1, l, result/cnt)
